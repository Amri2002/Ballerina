...
import { Plus, Trash2, Save, Download, Database, GitMerge, Key, Link2, CheckCircle } from "lucide-react";
...
const erExercises: Exercise[] = [
  {
    id: 1,
    title: "Basic Entity Design",
    description: "Create entities with proper attributes and primary keys",
    difficulty: "Beginner",
    scenario: "Design a library system",
    task: "Create a 'Book' entity with: book_id (PK), title, author, isbn, publication_year",
    solution: "Book entity with 5 attributes including primary key",
    hint: "Each entity needs a unique identifier (primary key) and descriptive attributes"
  },
  {
    id: 2,
    ...
    difficulty: "Beginner",
    scenario: "University system with students and courses",
    task: "Create Student and Enrollment entities. One student can have many enrollments.",
    solution: "1:M relationship from Student to Enrollment",
    hint: "Foreign key goes on the 'many' side of the relationship"
  },
  {
    id: 3,
    title: "Many-to-Many Relationship",
    description: "Design complex relationships with junction tables",
    difficulty: "Intermediate",
    scenario: "E-commerce with products and categories",
    task: "Model the relationship where products can belong to multiple categories and categories can have multiple products",
    solution: "Product_Category junction table with foreign keys",
    hint: "Many-to-many relationships require a bridge/junction table"
  }
];
...
export function ErDiagramDesigner() {
  ...
  const exercise = activeTab === "practice" ? erExercises[currentExercise] : null;
  ...
  const checkSolution = () => {
    ...
    switch (exercise.id) {
      case 1:
        isCorrect = entityCount >= 1;
        feedbackMessage = isCorrect 
          ? 'Great! You\'ve created the basic entity structure.'
          : 'You need to create at least one entity with proper attributes.';
        break;
      case 2:
        isCorrect = entityCount >= 2 && relationshipCount >= 1;
        feedbackMessage = isCorrect
          ? 'Perfect! You\'ve modeled the one-to-many relationship correctly.'
          : 'You need at least 2 entities connected with a relationship.';
        break;
      case 3:
        isCorrect = entityCount >= 3 && relationshipCount >= 2;
        feedbackMessage = isCorrect
          ? 'Excellent! You\'ve created the junction table for many-to-many relationship.'
          : 'Remember: many-to-many relationships need a junction table.';
        break;
      default:
        feedbackMessage = 'Keep working on your design!';
    }
  ...
  const nextExercise = () => {
    if (currentExercise < erExercises.length - 1) {
    ...
    <div className="space-y-6">
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="grid w-full grid-cols-2">
          ...
          <TabsTrigger value="designer">Free Design</TabsTrigger>
        </TabsList>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Badge variant="outline">
                      {currentExercise + 1} of {erExercises.length}
                    </Badge>
                  </div>
                </div>
              </CardHeader>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
            ...
            {exercise && (
              <Card className="lg:col-span-1">
                <CardHeader>
                  <CardTitle className="text-sm">Exercise Guide</CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="p-3 bg-muted/50 rounded-lg">
                    <h4 className="font-medium text-sm mb-2">Scenario:</h4>
                    <p className="text-xs">{exercise.scenario}</p>
                  </div>
                  
                  <div className="p-3 border rounded-lg">
                    <h4 className="font-medium text-sm mb-2">Task:</h4>
                    <p className="text-xs">{exercise.task}</p>
                  </div>

                  {showFeedback && (
                    <div className={`p-3 rounded-lg border ${
                      feedback.includes('Great') || feedback.includes('Perfect') || feedback.includes('Excellent')
                  ...
                  <div className="flex gap-2">
                    <Button onClick={checkSolution} size="sm" className="flex-1">
                      Check Solution
                    </Button>
                  </div>
                  ...
                  <div className="flex justify-between">
                    ...
                    <Button 
                      ...
                      disabled={currentExercise === erExercises.length - 1}
                    >
                      Next
                    </Button>
                  </div>
            ...
            {/* ER Diagram Canvas */}
            <div className="lg:col-span-3 h-[70vh] border rounded-lg overflow-hidden">
              <ReactFlow
                nodes={nodes}
                edges={edges}
